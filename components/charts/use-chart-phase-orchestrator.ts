...
export function useChartPhaseOrchestrator({
  ...
  const shouldEnterReveal = !skipEnterReveal && chartStatus === "ready"
  const [chartPhase, setChartPhase] = useState<ChartPhase>(() =>
    shouldEnterReveal ? "revealing" : resolveRestingChartPhase(chartStatus)
  )
  ...
  const [revealEpoch, setRevealEpoch] = useState(() =>
    shouldEnterReveal ? 1 : 0
  )
  const [concealEpoch, setConcealEpoch] = useState(0)
  const [isLoaded, setIsLoaded] = useState(
    () => chartStatus === "ready" && !shouldEnterReveal
  )
